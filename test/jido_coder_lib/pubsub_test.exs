defmodule JidoCoderLib.PubSubTest do
  use ExUnit.Case, async: false

  @moduledoc """
  Tests for the JidoCoderLib.PubSub wrapper module.
  """

  describe "pubsub_name/0" do
    test "returns the correct PubSub name" do
      assert JidoCoderLib.PubSub.pubsub_name() == :jido_coder_pubsub
    end
  end

  describe "topic prefixes" do
    test "agent_prefix/0 returns correct prefix" do
      assert JidoCoderLib.PubSub.agent_prefix() == "jido.agent"
    end

    test "session_prefix/0 returns correct prefix" do
      assert JidoCoderLib.PubSub.session_prefix() == "jido.session"
    end

    test "client_events_topic/0 returns correct topic" do
      assert JidoCoderLib.PubSub.client_events_topic() == "jido.client.events"
    end

    test "client_session_prefix/0 returns correct prefix" do
      assert JidoCoderLib.PubSub.client_session_prefix() == "jido.client.session"
    end

    test "signal_prefix/0 returns correct prefix" do
      assert JidoCoderLib.PubSub.signal_prefix() == "jido.signal"
    end

    test "protocol_prefix/0 returns correct prefix" do
      assert JidoCoderLib.PubSub.protocol_prefix() == "jido.protocol"
    end
  end

  describe "topic builders" do
    test "agent_topic/1 builds correct topic" do
      assert JidoCoderLib.PubSub.agent_topic("coordinator") == "jido.agent.coordinator"
      assert JidoCoderLib.PubSub.agent_topic("llm") == "jido.agent.llm"
    end

    test "session_topic/1 builds correct topic" do
      assert JidoCoderLib.PubSub.session_topic("session-123") == "jido.session.session-123"
    end

    test "client_session_topic/1 builds correct topic" do
      assert JidoCoderLib.PubSub.client_session_topic("abc") == "jido.client.session.abc"
    end

    test "signal_topic/1 builds correct topic" do
      assert JidoCoderLib.PubSub.signal_topic("file_changed") == "jido.signal.file_changed"
    end

    test "protocol_topic/1 builds correct topic" do
      assert JidoCoderLib.PubSub.protocol_topic("mcp") == "jido.protocol.mcp"
    end
  end
  ...
  describe "subscribe/2" do
    ...
    test "subscribes the current process to a topic" do
      topic = "test.topic"
      assert JidoCoderLib.PubSub.subscribe(topic) == :ok
      # Verify subscription by broadcasting and receiving
      JidoCoderLib.PubSub.broadcast(topic, :test_message)
    ...
    test "subscribes a specific process to a topic" do
      ...
      assert JidoCoderLib.PubSub.subscribe(child, "test.topic2") == :ok
    end
  end
  ...
  describe "subscribe helpers" do
    ...
    test "subscribe_client_events/0 subscribes to client events" do
      assert JidoCoderLib.PubSub.subscribe_client_events() == :ok
      JidoCoderLib.PubSub.broadcast_client_event(:test)
      # Messages from broadcast are wrapped as {sender, message}
      assert_receive {_, :test}
    after
      JidoCoderLib.PubSub.unsubscribe(JidoCoderLib.PubSub.client_events_topic())
    end

    test "subscribe_agent/1 subscribes to agent events" do
      assert JidoCoderLib.PubSub.subscribe_agent("coordinator") == :ok
      JidoCoderLib.PubSub.broadcast_agent("coordinator", :agent_event)
      assert_receive {_, :agent_event}
    after
      JidoCoderLib.PubSub.unsubscribe(JidoCoderLib.PubSub.agent_topic("coordinator"))
    end

    test "subscribe_session/1 subscribes to session events" do
      assert JidoCoderLib.PubSub.subscribe("jido.session.test123") == :ok
      JidoCoderLib.PubSub.broadcast_session("test123", :session_event)
      assert_receive {_, :session_event}
    after
      JidoCoderLib.PubSub.unsubscribe("jido.session.test123")
    end
  end
  ...
  describe "broadcast/3" do
    ...
    test "broadcasts a message to a topic" do
      ...
      JidoCoderLib.PubSub.subscribe(topic)

      JidoCoderLib.PubSub.broadcast(topic, :hello)
    ...
    test "broadcast includes the sender in the message tuple" do
      ...
      JidoCoderLib.PubSub.broadcast(topic, :message)
    ...
    test "broadcast with custom from option" do
      ...
      JidoCoderLib.PubSub.broadcast(topic, :message, from: custom_pid)
  ...
  describe "broadcast helpers" do
    ...
    test "broadcast_client_event/2 broadcasts to client events topic" do
      ...
      JidoCoderLib.PubSub.broadcast_client_event(:client_msg)
    ...
    test "broadcast_agent/2 broadcasts to agent topic" do
      JidoCoderLib.PubSub.subscribe_agent("test_agent")
      JidoCoderLib.PubSub.broadcast_agent("test_agent", :agent_msg)
      assert_receive {_, :agent_msg}
    after
      JidoCoderLib.PubSub.unsubscribe(JidoCoderLib.PubSub.agent_topic("test_agent"))
    end

    test "broadcast_session/2 broadcasts to session topic" do
      JidoCoderLib.PubSub.subscribe("jido.session.test_session")
      JidoCoderLib.PubSub.broadcast_session("test_session", :session_msg)
      assert_receive {_, :session_msg}
    after
      JidoCoderLib.PubSub.unsubscribe("jido.session.test_session")
    end

    test "broadcast_signal/2 broadcasts to signal topic" do
      JidoCoderLib.PubSub.subscribe("jido.signal.test_signal")
      JidoCoderLib.PubSub.broadcast_signal("test_signal", :signal_msg)
      assert_receive {_, :signal_msg}
    after
      JidoCoderLib.PubSub.unsubscribe("jido.signal.test_signal")
    end
  end
  ...
  describe "unsubscribe/1" do
    ...
    test "unsubscribes the current process from a topic" do
      ...
      JidoCoderLib.PubSub.subscribe(topic)
      assert JidoCoderLib.PubSub.unsubscribe(topic) == :ok

      # Should not receive message after unsubscribe
      JidoCoderLib.PubSub.broadcast(topic, :after_unsub)
  ...
  describe "multiple subscribers" do
    ...
    test "multiple processes receive broadcasts" do
      ...
      # Broadcast a message
      JidoCoderLib.PubSub.broadcast(topic, :broadcast_to_all)
  ...
  defp unsubscribe_all(_context) do
    ...
    topics = [
      ...
      JidoCoderLib.PubSub.client_events_topic(),
      JidoCoderLib.PubSub.agent_topic("coordinator"),
      JidoCoderLib.PubSub.agent_topic("test_agent"),
      JidoCoderLib.PubSub.session_topic("test_session"),
      JidoCoderLib.PubSub.signal_topic("test_signal")
    ]
