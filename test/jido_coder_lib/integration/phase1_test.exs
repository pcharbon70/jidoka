defmodule JidoCoderLib.Integration.Phase1Test do
  ...
  describe "Application Lifecycle" do
    ...
    test "all children are started" do
      ...
      assert Process.whereis(JidoCoderLib.AgentRegistry) != nil
      assert Process.whereis(JidoCoderLib.TopicRegistry) != nil
      assert Process.whereis(JidoCoderLib.SecureCredentials) != nil
      assert Process.whereis(JidoCoderLib.ContextStore) != nil
      assert Process.whereis(JidoCoderLib.Jido) != nil
      assert Process.whereis(JidoCoderLib.AgentSupervisor) != nil
      assert Process.whereis(JidoCoderLib.Agents.SessionManager) != nil
      assert Process.whereis(JidoCoderLib.ProtocolSupervisor) != nil
      assert Process.whereis(JidoCoderLib.Indexing.IndexingStatusTracker) != nil
    end
  ...
  describe "Registry Integration" do
    @agent_registry JidoCoderLib.AgentRegistry
  ...
  describe "ETS Integration" do
    ...
    test "concurrent analysis cache operations" do
      tasks =
        for i <- 1..10 do
          Task.async(fn ->
            ...
            result = %{index: i}
            JidoCoderLib.ContextStore.cache_analysis(file_path, :test, result)

            case JidoCoderLib.ContextStore.get_analysis(file_path, :test) do
              {:ok, %{index: ^i}} -> :ok
              _ -> :error
            end
          end)
      ...
      # Cleanup
      for i <- 1..10 do
        JidoCoderLib.ContextStore.invalidate_file("/test/concurrent_#{i}.ex")
      end
    end
    ...
    test "can invalidate cached analysis" do
      ...
      JidoCoderLib.ContextStore.cache_analysis(file_path, :test_analysis, result)
      assert {:ok, ^result} = JidoCoderLib.ContextStore.get_analysis(file_path, :test_analysis)

      JidoCoderLib.ContextStore.invalidate_file(file_path)
      assert :error = JidoCoderLib.ContextStore.get_analysis(file_path, :test_analysis)
    end

    test "can get cache statistics" do
      # Add some test data
      JidoCoderLib.ContextStore.cache_analysis("/test/stats1.ex", :test, %{})
      JidoCoderLib.ContextStore.cache_analysis("/test/stats2.ex", :test, %{})

      stats = JidoCoderLib.ContextStore.stats()
      assert is_map(stats)
  ...
  describe "Configuration Integration" do
    test "configuration loads for current environment" do
      operation_timeout = Application.get_env(:jido_coder_lib, :operation_timeout)
    ...
    test "configuration validation passes with valid config" do
      Application.put_env(:jido_coder_lib, :llm, provider: :mock, model: "test-model")
      ...
      assert :ok = JidoCoderLib.Config.validate_all()
    end

    test "configuration validation fails with invalid config" do
      Application.put_env(:jido_coder_lib, :llm, provider: :invalid_provider)
  ...
  describe "Concurrency" do
    test "concurrent registry operations" do
      ...
      processes =
        for i <- 1..20 do
          spawn(fn ->
            ...
            Registry.register(JidoCoderLib.AgentRegistry, key, i)
      ...
      Enum.each(registry_entries, fn {key, pid} ->
        assert [{^pid, _}] = Registry.lookup(JidoCoderLib.AgentRegistry, key)
      end)
      ...
      tasks =
        for i <- 1..20 do
          Task.async(fn ->
            file_path = "/test/concurrent_#{i}.ex"
            result = %{index: i}
            JidoCoderLib.ContextStore.cache_analysis(file_path, :concurrent_test, result)

            case JidoCoderLib.ContextStore.get_analysis(file_path, :concurrent_test) do
              {:ok, %{index: ^i}} -> :ok
              _ -> :error
            end
          end)
