defmodule JidoCoderLib.Integration.Phase3Test do
  ...
  alias JidoCoderLib.{Client, PubSub, Agents.ContextManager}
  ...
  describe "Session Isolation (3.8.2)" do
    ...
    test "ETS cache is isolated between sessions" do
      ...
      :ok = JidoCoderLib.ContextStore.cache_file(session_a, file_a, "content for A", %{})
      :ok = JidoCoderLib.ContextStore.cache_file(session_b, file_b, "content for B", %{})

      # Verify each session gets its own content
      {:ok, {content_a, _, _}} = JidoCoderLib.ContextStore.get_file(session_a, file_a)
      {:ok, {content_b, _, _}} = JidoCoderLib.ContextStore.get_file(session_b, file_b)
