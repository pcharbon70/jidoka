defmodule JidoCoderLib.TopicRegistryTest do
  use ExUnit.Case, async: false

  @moduledoc """
  Tests for the JidoCoderLib.TopicRegistry module.
  """

  describe "registry_name/0" do
    test "returns the correct registry name" do
      assert JidoCoderLib.TopicRegistry.registry_name() == JidoCoderLib.TopicRegistry
    end
  end
  ...
  describe "register/2" do
    test "registers the current process under a key" do
      ...
      assert {:ok, ^key} = JidoCoderLib.TopicRegistry.register(key)
      assert JidoCoderLib.TopicRegistry.registered?(key)

      JidoCoderLib.TopicRegistry.unregister(key)
    end
    ...
    test "allows multiple processes to register under the same key" do
      ...
      {:ok, ^key} = JidoCoderLib.TopicRegistry.register(key)
      ...
      assert JidoCoderLib.TopicRegistry.count(key) == 3
  ...
  describe "lookup/1" do
    test "returns all processes registered under a key" do
      ...
      assert {:ok, entries} = JidoCoderLib.TopicRegistry.lookup(key)
    ...
    test "returns empty list when key not found" do
      ...
      assert :error = JidoCoderLib.TopicRegistry.lookup(key)
    end
  end
  ...
  describe "unregister/1" do
    test "unregisters the current process from a key" do
      ...
      assert JidoCoderLib.TopicRegistry.count(key) == 2

      # Unregister current process
      JidoCoderLib.TopicRegistry.unregister(key)

      # Should still have 1 (the child)
      assert JidoCoderLib.TopicRegistry.count(key) == 1
  ...
  describe "dispatch/3" do
    test "sends a message to all registered processes" do
      ...
      # Dispatch a message
      assert {:ok, 3} = JidoCoderLib.TopicRegistry.dispatch(key, :broadcast)
    ...
    test "returns :error when no processes are registered" do
      ...
      assert :error = JidoCoderLib.TopicRegistry.dispatch(key, :test_message)
    end
    ...
    test "returns count of processes that received the message" do
      ...
      assert {:ok, 3} = JidoCoderLib.TopicRegistry.dispatch(key, :test)
    ...
    test "accepts custom from option" do
      ...
      JidoCoderLib.TopicRegistry.dispatch(key, :test_message, from: custom_pid)
  ...
  describe "count/1" do
    ...
    test "returns 0 when no process is registered" do
      ...
      assert JidoCoderLib.TopicRegistry.count(key) == 0
    end
  end
  ...
  describe "list_keys/0" do
    test "returns all unique registered keys" do
      ...
      {:ok, ^key1} = JidoCoderLib.TopicRegistry.register(key1)
      {:ok, ^key2} = JidoCoderLib.TopicRegistry.register(key2)

      keys = JidoCoderLib.TopicRegistry.list_keys()

      assert key1 in keys
      assert key2 in keys

      JidoCoderLib.TopicRegistry.unregister(key1)
      JidoCoderLib.TopicRegistry.unregister(key2)
    end
  end
  ...
  describe "registered?/1" do
    ...
    test "returns false when no process is registered" do
      ...
      refute JidoCoderLib.TopicRegistry.registered?(key)
    end
  end
  ...
  describe "register_multi/1" do
    test "registers the current process under multiple keys" do
      ...
      assert {:ok, [^key1, ^key2]} = JidoCoderLib.TopicRegistry.register_multi([key1, key2])

      assert JidoCoderLib.TopicRegistry.registered?(key1)
      assert JidoCoderLib.TopicRegistry.registered?(key2)
    ...
    test "returns error list when any registration fails" do
      ...
      assert {:ok, [^key1]} = JidoCoderLib.TopicRegistry.register_multi([key1])
