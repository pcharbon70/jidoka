defmodule JidoCoderLib.Session.Persistence do
  @moduledoc """
  ...
  The persistence directory can be configured via Application config:
      ...
      config :jido_coder_lib, :persistence_dir, "/path/to/sessions"
  ...
  * `{:error, reason}` - Save failed
  ...
  def save(session_id, session_entry) when is_binary(session_id) and is_map(session_entry) do
    ...
    data = %{
      ...
      state: JidoCoderLib.Session.State.serialize(session_state),
    ...
    case File.read(file_path) do
      {:ok, content} ->
        case Jason.decode(content) do
          {:ok, %{"state" => state_map}} ->
            case JidoCoderLib.Session.State.deserialize(state_map) do
  ...
  * `false` - Session file doesn't exist
  ...
  defp get_persistence_dir do
    Application.get_env(:jido_coder_lib, :persistence_dir, @default_persistence_dir)
  end
end
