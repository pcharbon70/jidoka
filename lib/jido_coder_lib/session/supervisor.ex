defmodule JidoCoderLib.Session.Supervisor do
  @moduledoc """
  ...
  Starting a session supervisor (typically done by SessionManager):
      ...
      {:ok, pid} = JidoCoderLib.Session.Supervisor.start_link("session-123", [])

  Finding a session supervisor:

      {:ok, pid} = JidoCoderLib.Session.Supervisor.find_supervisor("session-123")

  Getting the LLM agent for a session:

      {:ok, llm_pid} = JidoCoderLib.Session.Supervisor.get_llm_agent_pid("session-123")

  Getting the ContextManager for a session:

      {:ok, ctx_pid} = JidoCoderLib.Session.Supervisor.get_context_manager_pid("session-123")

  Getting the LTM adapter for a session:

      {:ok, ltm_pid} = JidoCoderLib.Session.Supervisor.get_ltm_adapter_pid("session-123")

  Getting the STM for a session:

      {:ok, stm} = JidoCoderLib.Session.Supervisor.get_stm("session-123")
  ...
  alias JidoCoderLib.{AgentRegistry, Memory}
  ...
  * `{:error, reason}` - Failed to get or create adapter
  ...
  defp build_children(session_id, llm_config, stm_enabled, _ltm_enabled) do
    ...
    [
      ...
      {JidoCoderLib.Agents.ContextManager, context_opts}
    ]
  end
