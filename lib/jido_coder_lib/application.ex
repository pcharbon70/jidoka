defmodule JidoCoderLib.Application do
  @moduledoc """
  The main application module for JidoCoderLib.
  ...
  ├── JidoCoderLib.Knowledge.Engine - added in Phase 5.2
  ├── JidoCoderLib.Indexing.IndexingStatusTracker - added in Phase 6.4.5
  ├── JidoCoderLib.Indexing.CodeIndexer - added in Phase 6.2
  ├── JidoCoderLib.ProtocolSupervisor (DynamicSupervisor)
  │   └── Protocol connections (MCP, Phoenix, A2A) - added in later phases
  ├── JidoCoderLib.AgentSupervisor (rest_for_one)
  │   └── Coordinator, CodeAnalyzer, IssueDetector - added in Phase 2.2+
  ├── JidoCoderLib.Agents.SessionManager - added in Phase 3.1
  ...
  def start(_type, _args) do
    children = [
      ...
      {JidoCoderLib.Knowledge.Engine,
       [
         name: :knowledge_engine,
         data_dir: knowledge_data_dir(),
         schema: Application.fetch_env!(:jido_coder_lib, :knowledge_engine)[:schema],
         health_check_interval:
           Application.fetch_env!(:jido_coder_lib, :knowledge_engine)[:health_check_interval]
       ]},
      # IndexingStatusTracker for code indexing operations (Phase 6.4.5)
      {JidoCoderLib.Indexing.IndexingStatusTracker,
       [name: JidoCoderLib.Indexing.IndexingStatusTracker, engine_name: :knowledge_engine]},
      ...
      # JidoCoderLib.TelemetryHandlers.attach_all() is called in Application.start
  ...
  defp knowledge_data_dir do
    case Application.get_env(:jido_coder_lib, :env, :dev) do
      :test ->
        Path.join([System.tmp_dir!(), "jido_kg_test"])

      _env ->
        Application.fetch_env!(:jido_coder_lib, :knowledge_engine)[:data_dir]
        |> Path.expand()
    end
  end
end
