defmodule JidoCoderLib.PubSub do
  @moduledoc """
  ...
  Phoenix.PubSub system used throughout JidoCoderLib.
  ...
  * `"jido.protocol.<protocol>"` - Protocol events
  ...
  ### Subscribe to a topic
      ...
      iex> JidoCoderLib.PubSub.subscribe(self(), "jido.agent.coordinator")
      :ok

  ### Broadcast to a topic

      iex> JidoCoderLib.PubSub.broadcast("jido.agent.coordinator", {:status, :ready})
      :ok

      # Or with from option
      iex> JidoCoderLib.PubSub.broadcast("jido.agent.coordinator", {:status, :ready}, from: self())
      :ok

  ### Subscribe to all client events

      iex> JidoCoderLib.PubSub.subscribe_client_events(self())
      :ok

  ### Broadcast to a session

      iex> JidoCoderLib.PubSub.broadcast_session("session-123", {:message, "hello"})
      :ok
  ...
  * `:metadata` - Optional metadata to attach to the subscription
  ...
  Subscribe the current process:
      ...
      iex> JidoCoderLib.PubSub.subscribe("jido.agent.coordinator")
      :ok
  ...
  Subscribe with options:
      ...
      iex> JidoCoderLib.PubSub.subscribe("jido.agent.coordinator", metadata: %{position: 1})
      {:ok, _}
