defmodule JidoCoderLib.Client do
  @moduledoc """
  ...
  Send a message to a session:
      ...
      {:ok, _history} = JidoCoderLib.Client.send_message(session_id, :user, "Hello!")
  ...
  * `{:context_updated, %{session_id: ...}}` - Context updated
  ...
  alias JidoCoderLib.Agents.{SessionManager, ContextManager}
  ...
  * `{:error, reason}` - Save failed
  ...
  def save_session(session_id) when is_binary(session_id) do
    with {:ok, session_info} <- SessionManager.get_session_info(session_id) do
      JidoCoderLib.Session.Persistence.save(session_id, %{state: session_info})
    end
  end
  ...
  * `{:error, reason}` - Restore failed
  ...
  def restore_session(session_id) when is_binary(session_id) do
    with {:ok, session_state} <- JidoCoderLib.Session.Persistence.load(session_id) do
  ...
  def list_saved_sessions do
    JidoCoderLib.Session.Persistence.list_saved()
  end
  ...
  * `{:error, reason}` - Delete failed
  ...
  def delete_saved_session(session_id) when is_binary(session_id) do
    JidoCoderLib.Session.Persistence.delete(session_id)
  end
end
