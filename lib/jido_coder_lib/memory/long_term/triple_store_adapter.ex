defmodule JidoCoderLib.Memory.LongTerm.TripleStoreAdapter do
  ...
  using the Jido ontology defined in `JidoCoderLib.Knowledge.Ontology`. All
  ...
  alias JidoCoderLib.Knowledge.{Engine, Context, Ontology, NamedGraphs}
  ...
  def new(session_id, opts \\ []) when is_binary(session_id) do
    with :ok <- Validation.validate_session_id(session_id) do
      default_engine =
        Application.get_env(:jido_coder_lib, :knowledge_engine_name, @default_engine)
