defmodule Jidoka.ContextStore do
  @moduledoc """
  ...
  * **Concurrency**: read_concurrency: true, write_concurrency: true
  ...
  Cache a file (global scope):
      ...
      Jidoka.ContextStore.cache_file("/path/to/file.ex", "content", %{language: :elixir})

  Cache a file for a specific session:

      Jidoka.ContextStore.cache_file("session-123", "/path/to/file.ex", "content", %{language: :elixir})

  Retrieve cached file:

      {:ok, {content, _mtime, _size}} = Jidoka.ContextStore.get_file("session-123", "/path/to/file.ex")

  Cache analysis result:

      Jidoka.ContextStore.cache_analysis("session-123", "/path/to/file.ex", :syntax_tree, ast)

  Retrieve cached analysis:

      {:ok, ast} = Jidoka.ContextStore.get_analysis("session-123", "/path/to/file.ex", :syntax_tree)

  Invalidate file cache for a session:

      Jidoka.ContextStore.invalidate_file("session-123", "/path/to/file.ex")

  Clear all cache for a session:

      Jidoka.ContextStore.clear_session_cache("session-123")
  ...
  * `:ok` - Successfully cached
  ...
  Jidoka.ContextStore.cache_file("/path/to/file.ex", "content")

  # Session-scoped cache
  Jidoka.ContextStore.cache_file("session-123", "/path/to/file.ex", "content")
  ...
  * `:error` - File not in cache
  ...
  # Global cache (backward compatible)
  {:ok, {content, mtime, size}} = Jidoka.ContextStore.get_file("/path/to/file.ex")

  # Session-scoped cache
  {:ok, {content, mtime, size}} = Jidoka.ContextStore.get_file("session-123", "/path/to/file.ex")
  ...
  * `:error` - Metadata not found
  ...
  ## Examples
      ...
      {:ok, meta} = Jidoka.ContextStore.get_metadata("session-123", "/path/to/file.ex")
  ...
  * `:ok` - Successfully cached
  ...
  ## Examples
      ...
      # Global cache (backward compatible)
      Jidoka.ContextStore.cache_analysis("/path/to/file.ex", :syntax_tree, ast)
  ...
  * `:error` - Analysis not found
  ...
  ## Examples
      ...
      # Global cache (backward compatible)
      {:ok, ast} = Jidoka.ContextStore.get_analysis("/path/to/file.ex", :syntax_tree)
  ...
  * `:error` - Analysis not found
  ...
  ## Examples
      ...
      # Global cache (backward compatible)
      {:ok, ast, timestamp} = Jidoka.ContextStore.get_analysis_with_timestamp("/path/to/file.ex", :syntax_tree)

      # Session-scoped cache
      {:ok, ast, timestamp} = Jidoka.ContextStore.get_analysis_with_timestamp("session-123", "/path/to/file.ex", :syntax_tree)
  ...
  * `:ok` - Successfully invalidated
  ...
  ## Examples
      ...
      # Global cache (backward compatible)
      Jidoka.ContextStore.invalidate_file("/path/to/file.ex")
  ...
  * Map with table sizes
  ...
  ## Examples
      ...
      [:file_content, :file_metadata, :analysis_cache] = Jidoka.ContextStore.table_names()
