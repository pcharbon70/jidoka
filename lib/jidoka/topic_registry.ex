defmodule Jidoka.TopicRegistry do
  @moduledoc """
  ...
  * `"topic:client:events"` - Client event broadcasts
  ...
  Register the current process under a topic:
      ...
      iex> Jidoka.TopicRegistry.register("topic:signal:file_changed")
      {:ok, "topic:signal:file_changed"}

  Look up all processes subscribed to a topic:

      iex> Jidoka.TopicRegistry.lookup("topic:signal:file_changed")
      {:ok, [{pid1, _}, {pid2, _}]}

  Dispatch a message to all processes under the topic:

      iex> Jidoka.TopicRegistry.dispatch("topic:signal:file_changed", {:changed, "/path/to/file"})
      {:ok, 2}

  Unregister the current process from a topic:

      iex> Jidoka.TopicRegistry.unregister("topic:signal:file_changed")
      :ok
  ...
  ## Examples
      ...
      iex> Jidoka.TopicRegistry.valid_key?("topic:signal:file_changed")
      true

      iex> Jidoka.TopicRegistry.valid_key?("signal:file_changed")
      false
  ...
  * `:key` - The registration key (defaults to the provided key argument)
  ...
  ## Examples
      ...
      iex> Jidoka.TopicRegistry.register("invalid-key")
      {:error, :invalid_key}
  ...
  * `:from` - The sender PID (defaults to self())
  ...
  ## Examples
      ...
      iex> Jidoka.TopicRegistry.dispatch("topic:signal:file_changed", {:changed, path})
      {:ok, 2}
  ...
  ## Examples
      ...
      iex> Jidoka.TopicRegistry.count("topic:signal:file_changed")
      3
  ...
  ## Examples
      ...
      iex> Jidoka.TopicRegistry.list_keys()
  ...
  ## Examples
      ...
      iex> Jidoka.TopicRegistry.registered?("topic:signal:file_changed")
      true
  ...
  ## Examples
      ...
      iex> Jidoka.TopicRegistry.register_multi(["topic:file:txt", "topic:file:ex"])
