defmodule Jidoka.Agents.SessionManager do
  @moduledoc """
  ...
  Sessions are tracked using `Jidoka.Session.State` structs which provide:
  ...
  Starting the SessionManager:
      ...
      {:ok, pid} = Jidoka.Agents.SessionManager.start_link()

  Creating a new session:

      {:ok, session_id} = Jidoka.Agents.SessionManager.create_session()

  Listing all sessions:

      sessions = Jidoka.Agents.SessionManager.list_sessions()

  Terminating a session:

      :ok = Jidoka.Agents.SessionManager.terminate_session(session_id)
  ...
  def handle_call({:create_session, opts}, _from, state) do
    ...
    case State.new(session_id, llm_config: llm_config, metadata: metadata) do
      {:ok, session_state} ->
        ...
        case Jidoka.Session.Supervisor.start_link(session_id, llm_config: llm_config) do
          {:ok, supervisor_pid} ->
